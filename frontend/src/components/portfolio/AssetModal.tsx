...
interface Props {
  ...
  /** Used in create mode to prefill from the watchlist. */
  preset?: AssetPreset
}

// Modal for adding or editing an asset. In create mode it can be seeded from a
// watchlist preset (ticker + name + current price) so the user only types qty.
export function AssetModal({ open, onClose, asset, preset }: Props) {
  ...
  // Live CAGR hint — fetched whenever the modal is open and ticker is non-empty.
  // 1-hour TTL on backend + react-query cache means repeated typing is free.
  const { data: metric } = useAssetMetric(open ? ticker : undefined)
  const cagrs = availableCAGRs(metric)

  // Reset/seed every time the modal opens so reopening doesn't show stale state
  useEffect(() => {
    if (!open) return
    if (asset) {
      setName(asset.name)
  ...
  // Convenience: when the user types qty AND we know a current price (preset),
  // auto-fill invested and value if those fields are still empty.
  useEffect(() => {
    if (isEdit || !preset?.currentPrice) return
    const q = Number(qty)
    if (!Number.isFinite(q) || q <= 0) return
    const calc = (q * preset.currentPrice).toFixed(2)
    setInvested((prev) => (prev === '' ? calc : prev))
    setValue((prev) => (prev === '' ? calc : prev))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [qty, preset?.currentPrice])
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (!Number.isFinite(i) || i < 0)  clientErrors.invested = '≥ 0'
    ...
    const body: AssetInputBody = {
      ...
      qty: q, invested: i, value: v, monthly: m,
      expectedReturn: r / 100,
    }
    try {
      if (isEdit && asset) {
        await update.mutateAsync({ id: asset.id, patch: body })
      } else {
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Editar ativo' : 'Adicionar ativo'} maxWidth={560}>
      {!isEdit && preset?.currentPrice && (
        <p className="muted modal-intro">
          Cotação atual de <strong>{preset.ticker}</strong>: {preset.currentPrice.toFixed(2)} USD.
          Indica a quantidade e os campos de investido/valor são pré-preenchidos.
        </p>
      )}
      <form onSubmit={submit} className="asset-form" noValidate>
        ...
        <div className="field-grid">
          ...
          <div className="field">
            <label htmlFor="a-invested">Investido (€)</label>
            <input
          ...
          <div className="field">
            <label htmlFor="a-value">Valor atual (€)</label>
            <input
