...
0. **Finish i18n (Phase 5)** — convert the last module to `t()` + fill its `pt`/`en` namespace: **Loan** (7 cmps) **+ Compare**. Done so far on `feat/i18n-en-pt`: infra + nav/auth/settings (P1) + Overview (P2) + Portfolio (P3) + Budget (P4, incl. `categoryLabel` display over canonical-pt stored values, plurals, import/bank flows). Follow the proven pattern (`useTranslation('<ns>')`, `<Trans>` for markup, keep pt/en parity — 622 keys so far).
