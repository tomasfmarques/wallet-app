...
- **Pushing to `main` deploys to prod** and runs `db:push:prod` — destructive on a column rename. Take a Neon snapshot before any rename/drop. **⚠️ Pending schema change (uncommitted):** `Expense.matchHint` (nullable, **additive** — safe; migration `add_expense_match_hint`). The next deploy's `db:push:prod` adds `expenses.match_hint` automatically; Neon snapshot first is good practice. (`Expense.loanId` from the prior commit is already live.)
