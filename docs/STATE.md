...
- **Pushing to `main` deploys to prod** and runs `db:push:prod` — destructive on a column rename. Take a Neon snapshot before any rename/drop. **⚠️ Pending schema change (uncommitted):** `User.pinHash` + new `WebAuthnCredential` table (migration `add_pin_and_webauthn`) — all **additive/safe**. The next deploy's `db:push:prod` adds `users.pin_hash` + `webauthn_credentials`; Neon snapshot first is good practice. (`Expense.loanId` + `Expense.matchHint` are already live.)
