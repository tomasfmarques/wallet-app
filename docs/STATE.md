...
- **Trading212 direct API — v2** (CSV import shipped `fad58ed`). Live "Sincronizar" sync: an **encrypted** per-user T212 key (new env `BROKER_ENC_KEY`, AES-GCM), `GET /equity/portfolio` + cached `/metadata/instruments` → reuse the **same** `POST /api/portfolio/import` pipeline + persisted `isin`. Mirror the GoCardless `bank.ts` pattern. Confirm at build time: auth model (single key vs key+secret), per-endpoint rate limits, EU-ETF ISIN resolution. See [`docs/decisions/portfolio.md`](docs/decisions/portfolio.md).
...
- **Pushing to `main` deploys to prod** and runs `db:push:prod` — destructive on a column rename. Take a Neon snapshot before any rename/drop. All recent schema changes are **additive**: `User.pinHash` + `WebAuthnCredential` (`add_pin_and_webauthn`), `User.isDemo`, `Expense.loanId`, `Expense.matchHint`, `PortfolioSettings.language`, **`Income.matchHint`** (`add_income_match_hint`, `9dce101`), and **`PortfolioAsset.isin`** (`add_portfolio_asset_isin`, `fad58ed`). All additive → `db:push:prod` adds the column on deploy, no snapshot needed.
