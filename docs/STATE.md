...
9. **Mortgage ↔ budget link** — connect the loan prestação to the budget plan/actual so it isn't triple-represented (pairs with #7). Likely a schema link.
10. **Yahoo failover (F8)** — serve a cached/fallback valuation when Yahoo breaks; wire in the Finnhub backup (needs `FINNHUB_API_KEY`). Backend, no schema.
11. **Privacy policy / data-safety drafts** — I can draft the text for #4 (hosting URLs + legal sign-off remain owner's).
