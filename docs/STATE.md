...
0. **Finish i18n (Phases 4–5)** — convert remaining modules to `t()` + fill their `pt`/`en` namespaces: **Budget** (11 cmps; exercises `categoryLabel` + plurals), **Loan + Compare**. Done so far on `feat/i18n-en-pt`: infra + nav/auth/settings (P1) + Overview (P2) + Portfolio (P3, incl. asset/search/reforçar modals + locale-aware `num`/`localeTag`). Follow the proven pattern (`useTranslation('<ns>')`, `<Trans>` for markup, keep pt/en parity — 411 keys so far).
