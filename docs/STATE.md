...
- **Pushing to `main` deploys to prod** and runs `db:push:prod` — destructive on a column rename. Take a Neon snapshot before any rename/drop. **⚠️ Pending schema changes (uncommitted):** `User.pinHash` + `WebAuthnCredential` table (`add_pin_and_webauthn`) **and** `User.isDemo` (`add_user_is_demo`) — all **additive/safe**. The next deploy's `db:push:prod` adds `users.pin_hash` + `webauthn_credentials`; Neon snapshot first is good practice. (`Expense.loanId` + `Expense.matchHint` are already live.)
